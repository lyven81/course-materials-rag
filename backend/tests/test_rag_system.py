...
class TestRAGSystem(unittest.TestCase):
    ...
    def test_rag_system_initialization(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_document_success(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
        ...
        # Verify processing
        rag_system.document_processor.process_course_document.assert_called_once_with(test_file)
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_document_error(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_folder_success(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr, mock_listdir, mock_exists, mock_isfile, mock_join):
        ...
        def mock_process_side_effect(file_path):
            if "course1" in file_path:
                course = Course(title="Course 1", instructor="Instructor 1", course_link="http://1.com", lessons=[])
                chunks = [CourseChunk(content="Content 1", course_title="Course 1", lesson_number=1, chunk_index=0)]
                return course, chunks
            elif "course2" in file_path:
                course = Course(title="Course 2", instructor="Instructor 2", course_link="http://2.com", lessons=[])
                chunks = [CourseChunk(content="Content 2", course_title="Course 2", lesson_number=1, chunk_index=0)]
                return course, chunks
            elif "course3" in file_path:
                course = Course(title="Course 3", instructor="Instructor 3", course_link="http://3.com", lessons=[])
                chunks = [CourseChunk(content="Content 3", course_title="Course 3", lesson_number=1, chunk_index=0)]
                return course, chunks
        
        rag_system.document_processor.process_course_document.side_effect = mock_process_side_effect
        
        # Execute
        total_courses, total_chunks = rag_system.add_course_folder(self.test_dir)
        
        # Verify processing (should skip readme.md)
        self.assertEqual(rag_system.document_processor.process_course_document.call_count, 3)
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_folder_nonexistent(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr, mock_exists):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_folder_skip_existing(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr, mock_listdir, mock_exists, mock_isfile, mock_join):
        ...
        def mock_process_side_effect(file_path):
            if "course1" in file_path:
                course = Course(title="Course 1", instructor="Instructor 1", course_link="http://1.com", lessons=[])  # Existing
                chunks = []
                return course, chunks
            elif "course2" in file_path:
                course = Course(title="Course 2", instructor="Instructor 2", course_link="http://2.com", lessons=[])  # New
                chunks = [CourseChunk(content="Content 2", course_title="Course 2", lesson_number=1, chunk_index=0)]
                return course, chunks
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_process_query_with_search_tool(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
        ...
        rag_system.ai_generator.generate_response.return_value = "Here are the search results about Python functions..."
        
        # Mock tool definitions  
        mock_search_def = {"name": "search_course_content", "description": "Search content"}
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_tool_integration(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
        ...
        rag_system.vector_store.search.return_value = Mock(error=None, documents=[], metadata=[], distances=[])
...
class TestRAGSystemIntegrationFlow(unittest.TestCase):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_content_search_flow(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
        ...
        mock_course = Course(
            ...
            lessons=[
                Lesson(lesson_number=1, title="Introduction", lesson_link="http://python.com/1")
            ]
        )
        mock_chunks = [CourseChunk(
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_outline_query_flow(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.DocumentProcessor')
    def test_error_handling_flow(self, mock_doc_proc, mock_vector_store, mock_ai_gen, mock_session_mgr):
